...
public class FileCopy {
    ...
    private static void verifySourceDir(File sourceDir) throws FileNotFoundException, InvalidObjectException {
        if (sourceDir.isDirectory()) {
            ...
            throw new FileNotFoundException("Directory %s does not exist.".format(sourceDir.getPath()));
        }
        ...
        for (File file : sourceDir.listFiles()) {
            ...
            if (!requiredFiles.remove(file.getName())) {
                throw new InvalidObjectException("Directory %s contains invalid element %s".format(sourceDir.getPath(), file.getName()));
            }
            // check for valid file type
            if (file.isDirectory()) {
                throw new InvalidObjectException("File %s is a directory. Must be a standard file.".format(file.getName()));
            }
        }
