...
class ForkManager {
    ...
    private Condition borhForkCondition;
    /**
    ...
    public ForkManager(int numForks, int delayTime) {
        ...
        this.borhForkCondition = mutex.newCondition();
    ...
    public void acquireForks(int philosopherId) throws InterruptedException {
        // TODO: Try-CAtch block wegen cleancode!
        this.mutex.lock();
        while (leftFork(philosopherId).state != FREE || rightFork(philosopherId).state != FREE) {
            this.borhForkCondition.await();
        }
